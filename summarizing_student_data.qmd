...
Second, read in and combine the data using functions from the tidyverse. Use an inner_join() on the variables they used in their code. Make a note of any  issues.
...
d_three
```
Issue noted: the above code join creates duplicate x and y verso
...
Use `mutate` to convert all identified variables to factors with appropriate options.
```{r}
students <-
  students |>
    mutate(across(all_of(factors_ord), ~factor(.x, ordered = T))) |>
